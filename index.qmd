...
As the catalogue matures, we plan to add detailed metadata for each work in MEI format [@MEI2023].


## Methods

We consulted the following sources, which contain partial catalogues of Werner's works:
...
```
In the resulting XML file, all tags in the `zs` namespace were removed, yielding the XML file `data/rism_entries.xml`, which only contains [MarcXML](https://www.loc.gov/standards/marcxml/) tags. From this file, searchable HTML tables were created via `script/parse_rism_search_results.ipynb` and `make_rism_tables.R` and stored in `data_generated/rism_tables/`.

Pages for each group of works were generated automatically from `catalogue.ods`. For each work, one incipit was generated from the Plaine & Easie Code of the first source with a RISM entry. Data from the [RISM Catalogue](https://opac.rism.info) by the *Répertoire International des Sources Musicales* is licensed under a Creative Commons Attribution CC-BY 3.0 Unported License.
...
I would appreciate your feedback! Feel free to reach out to me via [email](mailto:wolfgang@esser-skala.at).
