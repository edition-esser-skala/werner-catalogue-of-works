...
- *identification*
  ...
  - HarBu: thematic catalogues of works in the Esterházy music collection by @Harich1932, later transcribed by @Muranyi1997
...
- *language*
- *classification and keywords*: genre of the work; use UNIMARC codes for [musical forms](http://www.urfm.braidense.it/risorse/searchforms_en.php) (field 128)
...
Use RISM scoring abbreviations and UNIMARC codes for [medium of performance](http://www.urfm.braidense.it/risorse/searchmedium_en.php) (field 126):
...
- indicate solo instruments in the notes (e.g., “clno solo”); use “org solo (improvised)” for organ solos that are only indicated by a corresponding directive in the bass voice, but not written out for the right hand
- If the actual extent (i.e., number of played bars) is ambiguous (e.g., because repeats are uncluear in instrumental works), enter the number of distinct bars.
...
- location
  ...
  - shelfmark (should be unique within a library; if a single shelfmark comprises several works, add a colon followed by a consecutive number to “split the shelfmark”)
...
- Manually add bibliographic references and modern editions as XML code. Each entry is represented by a `<bibl>` element that contains two tags:
  - `<genre>`: reference, inventory, or edition
  - `<ref>`: points to a reference using [Pandoc Citations](https://pandoc.org/MANUAL.html#citation-syntax) format (also see [Quarto documentation](https://quarto.org/docs/authoring/citations.html)).
    ...
    <bibl>
      ...
      <ref targettype="pandoc-citation">@EsterhazyInv1805 [fol. 2r, no. 22]</ref>
    </bibl>
    <bibl>
      <genre>edition</genre>
